...
const initials = (name?: string, email?: string): string => {
  const source = name?.trim() || email?.trim() || 'CAD';
...
export const ShieldSidebar: React.FC<{
  ...
  <aside
    className={`relative hidden h-[100dvh] shrink-0 overflow-visible bg-primary-500 text-white shadow-xl transition-all duration-200 dark:bg-gray-900 md:block ${
      collapsed ? 'w-20' : 'w-72'
    }`}
  >
    <button
    ...
    <div className="shield-sidebar flex h-[100dvh] flex-col overflow-y-auto overflow-x-hidden">
      ...
      <div className={collapsed ? 'px-3 py-3' : 'px-4 py-3'}>
        <button
          type="button"
          onClick={onProfile}
          className={`w-full overflow-hidden rounded bg-white/10 text-left transition hover:bg-white/15 ${collapsed ? 'p-1.5' : 'p-2.5'}`}
          title="Profile"
        >
          <div className={collapsed ? 'flex justify-center' : 'flex items-center gap-3'}>
            <div
              className={`${collapsed ? 'h-10 w-10 text-sm' : 'h-12 w-12 text-base'} flex shrink-0 items-center justify-center rounded-full border border-white bg-white font-bold text-primary-500 shadow`}
            >
              {user ? initials(user.name, user.email) : <UserCircle size={28} />}
            </div>
            {!collapsed && (
          ...
          {!collapsed && (
            <div className="mt-2 rounded bg-black/15 px-3 py-1.5 text-xs font-semibold capitalize text-white">
              {user?.role || 'user'} - {user?.cadUnitNumber || user?.unitNumber || user?.badge || 'No unit'}
            </div>
          )}
        </button>
      </div>
      ...
      {footerItems.length > 0 && (
        <nav className="shrink-0 border-t border-white/10 px-3 py-3">
...
const SidebarButton: React.FC<{ item: ShieldSidebarItem; compact: boolean }> = ({ item, compact }) => {
  ...
  return (
    <button
      ...
      } ${item.active ? 'bg-white text-primary-500 shadow' : 'text-blue-50 hover:bg-white/10'}`}
